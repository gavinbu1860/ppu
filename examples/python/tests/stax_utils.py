...
def custom_mse_loss_fwd(y, label):
    ...
    print("y type: {0}, label type: {1}, loss type: {2}".format(
        y.dtype, label.dtype, loss.dtype))
    return loss, 2. * (y - label)
...
def custom_mse_loss_bwd(res, g):
    ...
    loss_hi = loss_lo
    # loss_hi = loss_lo
    print("loss_lo type: {0}, loss_hi type: {1}, g type: {2}".format(
...
def custom_Dot_fwd(inputs, W):
    ...
    print("inputs type: {0}, W_lo type: {1}, W type: {2}, output type: {3}".
          format(inputs.dtype, W_lo.dtype, W.dtype, outputs.dtype))
...
def custom_Dot_bwd(res, g):
    ...
    inputs_hi = inputs_lo
    print(
        "inputs_lo type: {0}, inputs_hi type: {1}, g type: {2}, W_hi type: {3}".
        format(inputs_lo.dtype, inputs_hi.dtype, g.dtype, W_hi.dtype))
