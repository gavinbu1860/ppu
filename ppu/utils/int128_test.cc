...
TEST(Int128Test, Decompose) {
  {
    int128_t v = 1;
    auto parts = ppu::DecomposeInt128(v);
    EXPECT_EQ(v, ppu::MakeInt128(parts.first, parts.second));
  }
  {
    int128_t v = -1;
    auto parts = ppu::DecomposeInt128(v);
  ...
  {
    uint128_t v = 1;
    auto parts = ppu::DecomposeUInt128(v);
    EXPECT_EQ(v, ppu::MakeUint128(parts.first, parts.second));
  }
}
