...
//
// Additionally, a ByteContainerView object can be constructed implicitly
// and cheaply from any other mutable or immutable byte-container objects such
// as another ByteContainerView object or a const instance of any other
// mutable byte-container object. The ByteContainerView object does not
// take ownership of the underlying memory. It is responsibility of the caller
// to make sure that the underlying memory referenced by the
// ByteContainerView remains valid for the lifetime of the
// ByteContainerView object.
...
class ByteContainerView {
 public:
  ...
  const uint8_t &at(size_t offset) const {
    PPU_ENFORCE(offset < size_, "Index out of bounds.");
    return data_[offset];
  }
  ...
  // The equality operator. Compares the contents of this
  // ByteContainerView with the contents of |other|.
  bool operator==(ByteContainerView other) const {
    return (size_ == other.size_) && (memcmp(data_, other.data_, size_) == 0);
  }

  // The inequality operator. Compares the contents of this
  // ByteContainerView with the contents of |other|.
