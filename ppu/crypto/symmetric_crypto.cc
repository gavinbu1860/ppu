...
const EVP_CIPHER* CreateEvpCipher(SymmetricCrypto::CryptoType type) {
  switch (type) {
    ...
    case SymmetricCrypto::CryptoType::SM4_CTR:
      return EVP_sm4_ctr();
    default:
      PPU_THROW("unknown crypto type: {}", static_cast<int>(type));
  }
}
...
EVP_CIPHER_CTX* CreateEVPCipherCtx(SymmetricCrypto::CryptoType type,
  ...
  PPU_ENFORCE(sizeof(key) == EVP_CIPHER_key_length(cipher));
  const uint8_t* key_data = reinterpret_cast<const uint8_t*>(&key);
  ...
  if ((type == SymmetricCrypto::CryptoType::AES128_ECB) ||
     ...
     */
    const uint8_t* iv_data = reinterpret_cast<const uint8_t*>(&iv);
    PPU_ENFORCE(
        EVP_CipherInit_ex(ctx, cipher, nullptr, key_data, iv_data, enc));
  }

  // No padding needed for aligned blocks.
  PPU_ENFORCE(EVP_CIPHER_CTX_set_padding(ctx, 0));
...
SymmetricCrypto::SymmetricCrypto(CryptoType type, uint128_t key, uint128_t iv)
  ...
  enc_ctx = CreateEVPCipherCtx(type_, key_, initial_vector_, 1);
  dec_ctx = CreateEVPCipherCtx(type_, key_, initial_vector_, 0);
}

SymmetricCrypto::SymmetricCrypto(CryptoType type, utils::ByteContainerView key,
...
void SymmetricCrypto::Decrypt(absl::Span<const uint8_t> ciphertext,
  ...
  if ((type_ != SymmetricCrypto::CryptoType::AES128_CTR) &&
    ...
    if (ciphertext.size() % BlockSize() != 0) {
      PPU_THROW("Requires size can be divided by block_size={}.", BlockSize());
    }
  }
  PPU_ENFORCE(plaintext.size() == ciphertext.size());
  ...
  if ((type_ == SymmetricCrypto::CryptoType::AES128_ECB) ||
    ...
    EVP_CIPHER_CTX_init(ctx);
    EVP_CIPHER_CTX_copy(ctx, dec_ctx);
  }
  ...
  while (left > 0) {
    ...
    PPU_ENFORCE(rc, "Fail to decrypt, rc={}", rc);
    i++;
    left -= n;
  }
...
void SymmetricCrypto::Encrypt(absl::Span<const uint8_t> plaintext,
  ...
  if ((type_ == SymmetricCrypto::CryptoType::AES128_ECB) ||
    ...
    ctx = EVP_CIPHER_CTX_new();
    EVP_CIPHER_CTX_init(ctx);
    EVP_CIPHER_CTX_copy(ctx, enc_ctx);
  }
  ...
  while (left > 0) {
    ...
    PPU_ENFORCE(rc, "Fail to encrypt, rc={}", rc);
    i++;
    left -= n;
  }
