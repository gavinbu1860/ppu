...
enum class AsymCryptoSchema : int { UNKNOWN, RSA2048_OAEP, RSA3072_OAEP, SM2 };
...
class AsymmetricEncryptor {
 public:
  ...
  virtual std::vector<uint8_t> Encrypt(utils::ByteContainerView plaintext) = 0;
};
...
class AsymmetricDecryptor {
 public:
  ...
  virtual std::vector<uint8_t> Decrypt(utils::ByteContainerView ciphertext) = 0;
};
