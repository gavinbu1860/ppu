...
const EVP_MD* CreateEvpMD(HashAlgorithm hash_algo_) {
  switch (hash_algo_) {
    ...
    default:
      PPU_THROW("Unsupported hash algo: {}", static_cast<int>(hash_algo_));
  }
}
...
SslHash& SslHash::Reset() {
  PPU_ENFORCE_EQ(EVP_MD_CTX_reset(context_), 1);
  int res = 0;
  const EVP_MD* md = CreateEvpMD(hash_algo_);
  res = EVP_DigestInit_ex(context_, md, nullptr);
  PPU_ENFORCE_EQ(res, 1, "EVP_DigestInit_ex failed.");

  return *this;
}

SslHash& SslHash::Update(utils::ByteContainerView data) {
  PPU_ENFORCE_EQ(EVP_DigestUpdate(context_, data.data(), data.size()), 1);
  return *this;
}
