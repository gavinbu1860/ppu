...
class SymmetricCrypto {
 public:
  ...
  SymmetricCrypto(CryptoType type, utils::ByteContainerView key,
...
template <typename T,
          std::enable_if_t<std::is_standard_layout<T>::value, int> = 0>
inline uint64_t FillAesRandom(SymmetricCrypto::CryptoType crypto_type,
                              uint128_t seed, uint128_t iv, uint64_t count,
                              absl::Span<T> out) {
  return FillPseudoRandom<T>(crypto_type, seed, iv, count, out);
}
