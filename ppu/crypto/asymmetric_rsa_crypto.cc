...
std::unique_ptr<RsaEncryptor> RsaEncryptor::CreateFromPem(
  ...
  PPU_ENFORCE(rsa, "No rsa from pem.");
  return std::unique_ptr<RsaEncryptor>(
      new RsaEncryptor(UniqueRsa(rsa, ::RSA_free)));
}
...
std::vector<uint8_t> RsaEncryptor::Encrypt(utils::ByteContainerView plaintext) {
  int buf_size = RSA_size(rsa_.get());
  PPU_ENFORCE_GT(buf_size, 0, "Illegal RSA_size.");
  PPU_ENFORCE_LT((int)plaintext.size() + kRsaInputSizeLimitOffset, buf_size,
                 "Invalid input size.");
  ...
  PPU_ENFORCE_GT(rc, 0, "Rsa encrypt error.");
...
std::unique_ptr<RsaDecryptor> RsaDecryptor::CreateFromPem(
  ...
  PPU_ENFORCE(rsa, "No rsa from string.");
...
std::vector<uint8_t> RsaDecryptor::Decrypt(
  ...
  PPU_ENFORCE_GE(rc, 0, "Rsa decrypt error.");
  plaintext.resize(rc);
  return plaintext;
}
