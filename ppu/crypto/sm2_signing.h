...
class Sm2Signer final : public crypto::AsymmetricSigner {
 public:
  ...
  static std::unique_ptr<Sm2Signer> CreateFromPem(
  ...
  std::vector<uint8_t> Sign(utils::ByteContainerView message,
                            utils::ByteContainerView id) const;

 private:
  Sm2Signer(UniquePkey pkey)
      : pkey_(std::move(pkey)),
...
class Sm2Verifier final : public crypto::AsymmetricVerifier {
 public:
  ...
  static std::unique_ptr<Sm2Verifier> CreateFromPem(
      utils::ByteContainerView sm2_pem);
  static std::unique_ptr<Sm2Verifier> CreateFromOct(
