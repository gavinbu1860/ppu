...
#include "ppu/crypto/dpf/serializable.pb.h"
...
uint8_t GetBit(DpfInStore x, uint32_t i) {
  PPU_ENFORCE(i < sizeof(DpfInStore) * 8, "GetBit: index out of range");
  return x >> i & 1;
}

DpfOutStore DpfPRG(uint128_t seed) {
  PseudoRandomGenerator<uint128_t, sizeof(uint128_t)> prng(seed);
  return prng();
}
...
std::tuple<uint128_t, bool, uint128_t, bool> SplitDpfSeed(uint128_t seed) {
  ...
  PseudoRandomGenerator<uint128_t, 3 * sizeof(uint128_t)> prng(seed);
...
void DpfContext::Gen(DpfKey& first_key, DpfKey& second_key, DpfInStore alpha,
                     ...
                     bool enable_evalall) {
  PPU_ENFORCE(this->in_bitnum_ > 0);
  PPU_ENFORCE(this->in_bitnum_ > log(alpha));
  PPU_ENFORCE(this->in_bitnum_ < 64);
  PPU_ENFORCE(this->ss_bitnum_ > 0);
  PPU_ENFORCE(this->ss_bitnum_ <= 64);

  // enable the early termination
  uint32_t term_level = GetTerminateLevel(enable_evalall);

  // set up the return keys
  first_key = DpfKey(0, GetInBitNum(), GetSsBitNum(), sec_param_, first_mk);
  second_key = DpfKey(1, GetInBitNum(), GetSsBitNum(), sec_param_, second_mk);
  ...
  t_working[0] = 0;  // default by definition
  t_working[1] = 1;  // default by definition
  ...
  PPU_ENFORCE(first_key.last_cw_vec.empty());
  PPU_ENFORCE(second_key.last_cw_vec.empty());
...
DpfOutStore DpfContext::Eval(DpfKey& key, DpfInStore x) {
  PPU_ENFORCE(this->in_bitnum_ > log(x));
  PPU_ENFORCE(key.enable_evalall == false);
...
void DpfContext::Traverse(DpfKey& key, std::vector<DpfOutStore>& result,
  ...
  if (current_level < term_level) {
    uint128_t seed_left, seed_right;
    bool t_left, t_right;
    ...
    uint64_t next_right_pos = (1 << current_level) + current_pos;
...
std::vector<DpfOutStore> DpfContext::EvalAll(DpfKey& key) {
  PPU_ENFORCE(key.enable_evalall == true);

  uint128_t seed_working = key.GetSeed();  // the initial value
  bool t_working = key.GetRank();          // the initial value
  uint32_t term_level = GetTerminateLevel(true);

  PPU_ENFORCE(GetInBitNum() <= 25);  // only support in_bin_num < 25

  uint64_t num = 1 << GetInBitNum();
