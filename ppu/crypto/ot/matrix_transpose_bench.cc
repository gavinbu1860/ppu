...
void GenerateRandomMatrix(std::array<uint128_t, 128>* matrix) {
  ...
  ppu::PseudoRandomGenerator<uint128_t> prg;
  prg.SetSeed(rd());
...
static void BM_EklundhTrans(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    state.ResumeTiming();
    for (size_t i = 0; i < n; i++) {
      ppu::EklundhTranspose128(&matrix);
    }
  }
}
...
static void BM_EklundhTrans1024(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    state.ResumeTiming();
    for (size_t i = 0; i < n; i++) {
      ppu::EklundhTranspose128x1024(&matrix);
    }
  }
}
...
static void BM_SseTrans1024(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    state.ResumeTiming();
    for (size_t i = 0; i < n; i++) {
      ppu::SseTranspose128x1024(&matrix);
    }
  }
}
