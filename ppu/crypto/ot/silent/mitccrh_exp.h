...
static inline void ParaEncExp(block* _blks, AES_KEY* keys) {
  ...
  for (unsigned int r = 0; r < 9; ++r) {
    ...
    for (int i = 0; i < numKeys; ++i) {
      ...
      int numEncs = 1 << i;
      for (int j = 0; j < numEncs; ++j, ++blks) *blks = vaeseq_u8(*blks, K);
    }
    blks = first;
    for (int i = 0; i < numKeys; ++i) {
      int numEncs = 1 << i;
      for (int j = 0; j < numEncs; ++j, ++blks) *blks = vaesmcq_u8(*blks);
    }
  }
  ...
  for (int i = 0; i < numKeys; ++i) {
    ...
    int numEncs = 1 << i;
    for (int j = 0; j < numEncs; ++j, ++blks) *blks = vaeseq_u8(*blks, K) ^ K;
  }
}
#endif
