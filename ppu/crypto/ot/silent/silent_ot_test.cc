...
#include "ppu/crypto/ot/silent/silent_ot.h"
...
double test_ot_cm_cc(SilentOT *ot, IO *ios[threads], int party,
  ...
  if (party == ALICE) {
    ot->send_ot_cm_cc(b0, b1, length);
  } else {
    ot->recv_ot_cm_cc(r, b, length);
  }
  ios[0]->flush();
...
double test_ot_rm_cc(SilentOT *ot, IO *ios[threads], int party,
  ...
  auto start = clock_start();
  if (party == ALICE) {
    ot->send_ot_rm_cc(b0, b1, length);
  } else {
    ot->recv_ot_rm_cc(r, b, length);
  }
  ios[0]->flush();
  long long t = time_from(start);
  ...
  if (party == ALICE) {
    ios[0]->send_block(b0, length);
    ios[0]->send_block(b1, length);
  } else if (party == BOB) {
    ios[0]->recv_block(b0, length);
    ios[0]->recv_block(b1, length);
    for (int64_t i = 0; i < length; ++i) {
      if (b[i])
...
double test_ot_rcm_cc(SilentOT *ot, IO *ios[threads], int party,
  ...
  auto start = clock_start();
  if (party == ALICE) {
    ot->send_ot_rcm_cc(b0, length);
    delta = ot->ferret->Delta;
  } else {
    ot->recv_ot_rcm_cc(r, b, length);
  }
  ios[0]->flush();
  long long t = time_from(start);
...
double test_ot_rm_rc(SilentOT *ot, IO *ios[threads], int party,
  ...
  auto start = clock_start();
  if (party == ALICE) {
    ot->send_ot_rm_rc(b0, b1, length);
  } else {
    ot->recv_ot_rm_rc(r, b, length);
  }
  ios[0]->flush();
  long long t = time_from(start);
...
double test_ot_cm_cc(SilentOT *ot, IO *ios[threads], int party,
                     int64_t length) {
  typedef typename std::conditional<(l <= 8), uint8_t, uint64_t>::type T;
  T **msgs = new T *[length];
  T *r = new T[length];
  ...
  PRG prg2;
  prg2.random_bool((bool *)b, length);

  uint64_t sent = total_sent(ios);

  auto start = clock_start();
  if (party == ALICE) {
    ot->send_ot_cm_cc<T>(msgs, length, l);
  } else {
    ot->recv_ot_cm_cc<T>(r, b, length, l);
  }
  ios[0]->flush();
  long long t = time_from(start);
...
double test_ot_cm_cc(SilentOT *ot, IO *ios[threads], int party,
  ...
  auto start = clock_start();
  if (party == ALICE) {
    ot->send_ot_cm_cc<T>(msgs, length, N, l);
  } else {
    ot->recv_ot_cm_cc<T>(r, b, length, N, l);
  }
  ios[0]->flush();
  long long t = time_from(start);
...
double test_ot_rm_rc(SilentOT *ot, IO *ios[threads], int party,
                     int64_t length) {
  typedef typename std::conditional<(l <= 8), uint8_t, uint64_t>::type T;
  T *msg0 = new T[length], *msg1 = new T[length];
  T *r = new T[length];
  ...
  auto start = clock_start();
  if (party == ALICE) {
    ot->send_ot_rm_rc<T>(msg0, msg1, length, l);
  } else {
    ot->recv_ot_rm_rc<T>(r, b, length, l);
  }
  ios[0]->flush();
  long long t = time_from(start);
  ...
  if (party == ALICE) {
    ios[0]->send_data(msg0, length * sizeof(T));
    ios[0]->send_data(msg1, length * sizeof(T));
  } else if (party == BOB) {
    ios[0]->recv_data(msg0, length * sizeof(T));
    ios[0]->recv_data(msg1, length * sizeof(T));
    for (int64_t i = 0; i < length; ++i) {
...
void test_silent(int party, IO *ios[threads], int64_t num_ot) {
  ...
  auto start = clock_start();
  SilentOT *silent_ot = new SilentOT(
  ...
  // batch
  t = test_ot_cm_cc(silent_ot, ios, party, 1);

  t = test_ot_cm_cc(silent_ot, ios, party, num);
  std::cout << (party == ALICE ? "[ALICE]\t" : "[BOB]\t")
            << "Silent OT (cm, cc):\t" << double(num) / t * 1e6 << " OTps"
            << std::endl;

  t = test_ot_rm_cc(silent_ot, ios, party, num);
  std::cout << (party == ALICE ? "[ALICE]\t" : "[BOB]\t")
            << "Silent OT (rm, cc):\t" << double(num) / t * 1e6 << " OTps"
            << std::endl;

  t = test_ot_rcm_cc(silent_ot, ios, party, num);
  std::cout << (party == ALICE ? "[ALICE]\t" : "[BOB]\t")
            << "Silent OT (rcm, cc):\t" << double(num) / t * 1e6 << " OTps"
            << std::endl;

  t = test_ot_rm_rc(silent_ot, ios, party, num);
  std::cout << (party == ALICE ? "[ALICE]\t" : "[BOB]\t")
            << "Silent OT (rm, rc):\t" << double(num) / t * 1e6 << " OTps"
            << std::endl;

  t = test_ot_cm_cc<2>(silent_ot, ios, party, num);
  std::cout << (party == ALICE ? "[ALICE]\t" : "[BOB]\t")
            << "Silent OT (cm, cc, l=2):\t" << double(num) / t * 1e6 << " OTps"
            << std::endl;

  t = test_ot_cm_cc<16, 2>(silent_ot, ios, party, num);
  std::cout << (party == ALICE ? "[ALICE]\t" : "[BOB]\t")
            << "Silent OT (cm, cc, N=16, l=2):\t" << double(num) / t * 1e6
            << " OTps" << std::endl;

  t = test_ot_cm_cc<16, 1>(silent_ot, ios, party, num);
  std::cout << (party == ALICE ? "[ALICE]\t" : "[BOB]\t")
            << "Silent OT (cm, cc, N=16, l=1):\t" << double(num) / t * 1e6
            << " OTps" << std::endl;

  t = test_ot_rm_rc<1>(silent_ot, ios, party, num);
  std::cout << (party == ALICE ? "[ALICE]\t" : "[BOB]\t")
...
TEST(SilentOTTest, Test) {
  ...
  std::future<void> alice = std::async([&] {
    ...
    for (int i = 0; i < threads; ++i) {
      ios[i] = new CheetahIo(contexts[0]);
    }
  ...
  std::future<void> bob = std::async([&] {
    CheetahIo *ios[threads];
    for (int i = 0; i < threads; ++i) {
      ios[i] = new CheetahIo(contexts[1]);
    }
