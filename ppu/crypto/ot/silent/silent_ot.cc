...
void SilentOT::recv_ot_cam_cc(T* data, const bool* b, int64_t length) {
  ...
  for (int64_t i = 0; i < length; i += ot_bsize) {
    ...
    memcpy(pad, rcm_data + i, corrected_bsize * sizeof(block));
    ...
    for (int j = i; j < i + ot_bsize and j < length; ++j) {
      ...
      if (b[j]) data[j] = corr_data[j - i] - data[j];
    }
  }
...
void SilentOT::send_ot_cm_cc(T** data, int64_t length, int l) {
  ...
  for (int64_t i = 0; i < length; i += ot_bsize) {
    ...
    pack_ot_messages<T>((T*)y, data + i, pad, corrected_y_size, corrected_bsize,
                        l, 2);
...
void SilentOT::send_ot_cm_cc(T** data, int64_t length, int N, int l) {
  ...
  for (int64_t i = 0; i < length; i += ot_bsize) {
    ...
    for (int64_t j = i; j < std::min(i + ot_bsize, length); ++j) {
      mitccrh_exp.renew_ks(rm_data0 + j * logN, logN);
      mitccrh_exp.hash_exp(hash_out, hash_in0, logN);
      mitccrh_exp.renew_ks(rm_data1 + j * logN, logN);
      mitccrh_exp.hash_exp(hash_out + N - 1, hash_in1, logN);
...
void SilentOT::recv_ot_cm_cc(T* data, const uint8_t* r, int64_t length, int N,
  ...
  for (int64_t i = 0; i < length; i += ot_bsize) {
    ...
    for (int64_t j = i; j < std::min(i + ot_bsize, length); ++j) {
      ...
      mitccrh_exp.hash_single(hash_out, hash_in, logN);
