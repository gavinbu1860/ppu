...
std::vector<uint128_t> GenerateRandomUint128(size_t n) {
  ...
  for (size_t i = 0; i < n; i++) {
    rand_vec[i] = ppu::MakeUint128(random(), random());
  }
  return rand_vec;
}

std::vector<ppu::block> GenerateRandomBlock(size_t n) {
  std::random_device rd;
  std::seed_seq seed({rd(), rd(), rd(), rd(), rd(), rd(), rd(), rd()});
  std::mt19937 random = std::mt19937(seed);

  std::vector<ppu::block> rand_vec(n + 1);
  for (size_t i = 0; i < n; i++) {
    rand_vec[i] = ppu::block(ppu::MakeUint128(random(), random()));
  }
  return rand_vec;
}
}  // namespace
...
static void BM_BlockMoveMaskEpi8(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::vector<ppu::block> a = GenerateRandomBlock(n);
    std::vector<int> c(n);
...
static void BM_Uint128MoveMaskEpi8(benchmark::State& state) {
  for (auto _ : state) {
    ...
    state.ResumeTiming();
    for (size_t i = 0; i < n; i++) {
      benchmark::DoNotOptimize(c[i] = ppu::block(a[i]).movemask_epi8());
    }
  }
}
...
static void BM_BlockAdd(benchmark::State& state) {
  for (auto _ : state) {
    ...
    size_t n = state.range(0);
    std::vector<ppu::block> a = GenerateRandomBlock(n);
    std::vector<ppu::block> b = GenerateRandomBlock(n);
    std::vector<ppu::block> c(n);
...
static void BM_BlockXor(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::vector<ppu::block> a = GenerateRandomBlock(batch_size);
    std::vector<ppu::block> b = GenerateRandomBlock(batch_size);
...
namespace {
std::vector<std::vector<ppu::block>> GenerateMatrixBlock(size_t n) {
  std::random_device rd;
  std::seed_seq seed({rd(), rd(), rd(), rd(), rd(), rd(), rd(), rd()});
  std::mt19937 random = std::mt19937(seed);

  std::vector<std::vector<ppu::block>> a(n);

  for (size_t i = 0; i < n; ++i) {
    a[i].resize(n);
    for (size_t j = 0; j < n; ++j) {
      a[i][j] = ppu::block(ppu::MakeUint128(random(), random()));
    }
  }
  return a;
}
...
std::vector<std::vector<uint128_t>> GenerateMatrixUint128(size_t n) {
  ...
  for (size_t i = 0; i < n; ++i) {
    a[i].resize(n);
    for (size_t j = 0; j < n; ++j) {
      a[i][j] = ppu::MakeUint128(random(), random());
    }
  }
  return a;
}
...
static void BM_BlockMatrixXor(benchmark::State& state) {
  for (auto _ : state) {
    ...
    std::vector<std::vector<ppu::block>> a, b, c(n);
