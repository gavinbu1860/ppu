...
class IGroupPRF {
 public:
  ...
  virtual uint128_t Eval(size_t group_idx, uint128_t input) const = 0;
...
class KkrtOtExtSender {
 public:
  ...
  void RecvCorrection(const std::shared_ptr<link::Context>& ctx);
  void SetCorrection(const std::string& recvceived_correction,
                     uint64_t recv_count);

  void Encode(uint64_t ot_idx, const uint128_t input, void* dest,
              uint64_t dest_ize);
...
class KkrtOtExtReceiver {
 public:
  ...
  void Encode(uint64_t ot_idx, const uint128_t input,
              absl::Span<uint8_t> dest_encode);
  void ZeroEncode(uint64_t ot_idx);

  void SendCorrection(const std::shared_ptr<link::Context>& ctx,
                      uint64_t send_count);
  std::string ShiftCorrection(uint64_t send_count);
