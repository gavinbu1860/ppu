...
const static uint128_t trans_mask_inv[7] = {
...
void EklundhTranspose128x1024(
  ...
  for (uint64_t i = 0; i < 8; ++i) {
    ...
    for (uint64_t j = 0; j < 128; ++j) sub[j] = (*inout)[j][i];

    EklundhTranspose128(&sub);

    for (uint64_t j = 0; j < 128; ++j) (*inout)[j][i] = sub[j];
  }
}
...
inline void SseLoadSubSquarex(std::array<std::array<block, 8>, 128>& in,
                              ...
                              uint64_t i) {
  typedef std::array<std::array<uint8_t, 16>, 2> OUT_t;
  typedef std::array<std::array<uint8_t, 128>, 128> IN_t;

  static_assert(sizeof(OUT_t) == sizeof(std::array<block, 2>), "");
  static_assert(sizeof(IN_t) == sizeof(std::array<std::array<block, 8>, 128>),
                "");

  OUT_t& out_byte_view = *(OUT_t*)&out;
  IN_t& in_byte_view = *(IN_t*)&in;
...
inline void SseTransposeSubSquarex(std::array<std::array<block, 8>, 128>& out,
  ...
  std::array<std::array<uint16_t, 64>, 128>& out_u16_view =
      *(std::array<std::array<uint16_t, 64>, 128>*)&out;
...
void SseTranspose128x1024(std::array<std::array<uint128_t, 8>, 128>* inout) {
  ...
  for (uint64_t i = 0; i < 128; i++) {
    (*inout)[i][0] = (uint128_t)matrix_block[i][0].mData;
    (*inout)[i][1] = (uint128_t)matrix_block[i][1].mData;
    (*inout)[i][2] = (uint128_t)matrix_block[i][2].mData;
    (*inout)[i][3] = (uint128_t)matrix_block[i][3].mData;
    (*inout)[i][4] = (uint128_t)matrix_block[i][4].mData;
    (*inout)[i][5] = (uint128_t)matrix_block[i][5].mData;
    (*inout)[i][6] = (uint128_t)matrix_block[i][6].mData;
    (*inout)[i][7] = (uint128_t)matrix_block[i][7].mData;
  }
}
