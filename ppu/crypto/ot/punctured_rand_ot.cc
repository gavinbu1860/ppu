...
#include "ppu/crypto/ot/punctured_rand_ot.h"
...
inline Buffer BufferFromArray(const std::array<T, N>& array) {
  return {reinterpret_cast<const char*>(array.data()),
          static_cast<int64_t>(array.size() * sizeof(T))};
}
...
inline Buffer BufferFromVectorBool(const std::vector<bool>& vector) {
  PPU_ENFORCE_GT(sizeof(uint32_t) * 16, vector.size());
  PPU_ENFORCE_GT(vector.size(), (uint32_t)1);
...
void PuncturedROTRecv(const std::shared_ptr<link::Context>& ctx,
  ...
  PPU_ENFORCE_GT(n, (uint32_t)1);
  PPU_ENFORCE_GE(ot_options.choices.size(), ot_num);
  PPU_ENFORCE_GE(ot_options.blocks.size(), ot_num);
  ...
  for (uint32_t i = 0, empty_pos = 0; i < ot_num; i++) {
    ...
    for (uint32_t j = 0; j < iter_num; j++) {
      ...
      PPU_ENFORCE_EQ(temp_seed_vec.empty(), 0);
...
void PuncturedROTSend(const std::shared_ptr<link::Context>& ctx,
  ...
  for (uint32_t i = 0; i < ot_num; i++) {
    ...
    ctx->SendAsync(ctx->NextRank(), BufferFromArray(send_message),
