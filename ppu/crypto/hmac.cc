...
void Init_HMAC(HashAlgorithm hash_algo, utils::ByteContainerView key,
               HMAC_CTX* context) {
  int res = 0;
  switch (hash_algo) {
    ...
    default:
      PPU_THROW("Unsupported hash algo: {}", static_cast<int>(hash_algo));
      break;
  }

  PPU_ENFORCE_EQ(res, 1, "Failed to HMAC_Init_ex.");
}

}  // namespace

Hmac::Hmac(HashAlgorithm hash_algo, utils::ByteContainerView key)
...
Hmac& Hmac::Reset() {
  PPU_ENFORCE_EQ(HMAC_CTX_reset(context_), 1);
  Init_HMAC(hash_algo_, key_, context_);
  return *this;
}

Hmac& Hmac::Update(utils::ByteContainerView data) {
  HMAC_Update(context_, data.data(), data.size());
  return *this;
}
...
std::vector<uint8_t> Hmac::CumulativeMac() const {
  ...
  PPU_ENFORCE(context_snapshot != nullptr);
  ON_SCOPE_EXIT([&] { HMAC_CTX_free(context_snapshot); });
  Init_HMAC(hash_algo_, key_, context_snapshot);
  PPU_ENFORCE_EQ(HMAC_CTX_copy(context_snapshot, context_), 1);
  size_t mac_size = HMAC_size(context_snapshot);
  PPU_ENFORCE_GT(mac_size, (size_t)0);
  std::vector<uint8_t> mac(mac_size);
  unsigned int len;
  PPU_ENFORCE_EQ(HMAC_Final(context_snapshot, mac.data(), &len), 1);
