...
#include "ppu/crypto/hmac_sha256.h"
...
class HmacTest : public testing::Test {
 ...
 protected:
  const TestData& Data() {
    ...
    PPU_THROW("Unsupported typename!");
  }
...
TYPED_TEST(HmacTest, TestVector1) {
  std::vector<uint8_t> mac =
      TypeParam(utils::ByteContainerView(this->Data().key))
...
TYPED_TEST(HmacTest, MultipleUpdates) {
  TypeParam hmac(utils::ByteContainerView(this->Data().key));
