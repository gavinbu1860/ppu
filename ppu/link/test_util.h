...
inline std::vector<std::shared_ptr<Context>> SetupWorld(const std::string& id,
  ...
  for (size_t rank = 0; rank < world_size; rank++) {
    ctx_desc.parties.push_back({"dummy_id", "dummy_host"});
  }
...
inline Buffer MakeRoundData(size_t rank, size_t round) {
  // result in different content/length for each rank/round.
  const auto spaces = std::string(rank, '_');
  auto str = fmt::format("d:{},{},r:{}", rank, spaces, round);
  return {str.c_str(), static_cast<int64_t>(str.size())};
}
