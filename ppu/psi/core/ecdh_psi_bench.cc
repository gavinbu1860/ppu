...
std::optional<ppu::CurveType> GetOverrideCurveType() {
  if (const auto* env = std::getenv("OVERRIDE_CURVE")) {
    if (std::strcmp(env, "25519") == 0) {
      return ppu::CurveType::Curve25519;
    }
    if (std::strcmp(env, "FOURQ") == 0) {
      return ppu::CurveType::CurveFourQ;
    }
  }
  return {};
}
...
static void BM_EcdhPsi(benchmark::State& state) {
  for (auto _ : state) {
    ...
    auto ctxs = ppu::link::test::SetupWorld(2);
    auto proc = [](std::shared_ptr<ppu::link::Context> ctx,
                   const std::vector<std::string>& items,
                   size_t target_rank) -> std::vector<std::string> {
      ppu::psi::PsiOptions options;
      auto memory_store = std::make_shared<ppu::psi::MemoryCipherStore>();
      {
        const auto curve = GetOverrideCurveType();
        options.ecc_cryptor = ppu::CreateEccCryptor(
            curve.has_value() ? *curve : ppu::CurveType::Curve25519);
        options.batch_provider =
            std::make_shared<ppu::psi::MemoryBatchProvider>(items);
        options.cipher_store = memory_store;
        options.link_ctx = ctx;
        options.target_rank = target_rank;
        options.ecc_cryptor = CreateEccCryptor(ppu::CurveType::Curve25519);
      }

      ppu::psi::RunEcdhPsi(options);
