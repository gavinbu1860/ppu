...
ShuffleEcdhPSI3Party::ShuffleEcdhPSI3Party(
    const std::shared_ptr<link::Context>& link, size_t master_rank,
...
void ShuffleEcdhPSI3Party::RunEcdhPsiStep1() {
  std::shared_ptr<link::Context> link2 = link_->Spawn();
...
void ShuffleEcdhPSI3Party::RunEcdhPsiStep2(
  ...
  if (link_->Rank() == master_rank_) {
    ...
    PPU_ENFORCE_EQ(step2_ack_str, step2_finish_ack_str);
  } else if (link_->PrevRank() == master_rank_) {
    ...
    link_->SendAsync(master_rank_,
                     {step2_finish_ack_str.c_str(),
                      static_cast<int64_t>(step2_finish_ack_str.size())},
