...
class ThrottleControlSender {
 public:
  ThrottleControlSender(const std::shared_ptr<link::Context>& link_send,
                        const std::shared_ptr<link::Context>& link_recv,
  ...
  void SendAsync(const Buffer& value, std::string_view tag);
 ...
 private:
  ...
  std::shared_ptr<link::Context> link_send_;
  // link_recv_ used to receive peer's received count number.
  std::shared_ptr<link::Context> link_recv_;
...
class ThrottleControlReceiver {
 public:
  ThrottleControlReceiver(
      const std::shared_ptr<link::Context>& link_recv_msg,
      const std::shared_ptr<link::Context>& link_send_counter)
      : link_recv_msg_(link_recv_msg), link_send_counter_(link_send_counter) {}

  Buffer RecvMsgWithSendCounter();

 private:
  // link_recv_msg_ used to receive real messsage.
  std::shared_ptr<link::Context> link_recv_msg_;
  // link_send_counter_ used to send received message count.
  std::shared_ptr<link::Context> link_send_counter_;
