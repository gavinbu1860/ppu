...
#include "ppu/crypto/ot/iknp_ot_extension.h"
#include "ppu/crypto/ot/kkrt_ot_extension.h"
#include "ppu/crypto/utils.h"
#include "ppu/psi/core/cuckoo_index.h"
#include "ppu/psi/core/throttle_control_link.h"
...
struct KkrtPsiBatch {
  ...
  static KkrtPsiBatch Deserialize(const Buffer& buf) {
    KkrtPsiBatchProto proto;
...
//
size_t ExchangeSetSize(const std::shared_ptr<link::Context>& link_ctx,
                       size_t items_size) {
...
std::shared_ptr<KkrtPsiRunContext> CreateReceiverRunContext(
  ...
  PPU_ENFORCE(cuckoo_index != nullptr);
  ...
  PPU_ENFORCE((*cuckoo_index).stash().empty(), "stash size not 0");
...
void GetKkrtOtSenderOptions(const std::shared_ptr<link::Context>& link_ctx,
                            const size_t num_ot, BaseRecvOptions* recv_opts) {
  PPU_ENFORCE(recv_opts != nullptr);
  size_t base_ot_num = 128;

  // use baseot get 128 ots
  BaseSendOptions iknp_send_opts;

  iknp_send_opts.blocks.resize(base_ot_num);

  BaseOtSend(link_ctx, absl::MakeSpan(iknp_send_opts.blocks));

  (*recv_opts).choices = CreateRandomChoices(num_ot);
  ...
  IknpOtExtRecv(link_ctx, iknp_send_opts, absl::MakeConstSpan(choices),
...
void GetKkrtOtReceiverOptions(const std::shared_ptr<link::Context>& link_ctx,
                              const size_t num_ot, BaseSendOptions* send_opts) {
  PPU_ENFORCE(send_opts != nullptr);
  size_t base_ot_num = 128;

  // use baseot get 128 ots
  BaseRecvOptions iknp_recv_opts;

  iknp_recv_opts.choices = CreateRandomChoices(base_ot_num);

  iknp_recv_opts.blocks.resize(base_ot_num);

  BaseOtRecv(link_ctx, iknp_recv_opts.choices,
...
void KkrtPsiSend(const std::shared_ptr<link::Context>& link_ctx,
  ...
  PPU_ENFORCE((kkrt_psi_options.cuckoo_hash_num == 3) &&
                  (kkrt_psi_options.stash_size == 0),
              "now only support cuckoo HashNum = 3 , stash size = 0");

  PPU_ENFORCE((base_options.blocks.size() == 512) &&
  ...
  PPU_ENFORCE((peer_set_size > 0) && (!items_hash.empty()),
  ...
  ctx->link0->SendAsync(ctx->link0->NextRank(),
                        {finish_str, static_cast<int64_t>(strlen(finish_str))},
                        fmt::format("KKRT:PSI:Finished"));
}

std::vector<std::size_t> KkrtPsiRecv(
    const std::shared_ptr<link::Context>& link_ctx,
    const KkrtPsiOptions& kkrt_psi_options, const BaseSendOptions& base_options,
    const std::vector<uint128_t>& items_hash) {
  PPU_ENFORCE((kkrt_psi_options.cuckoo_hash_num == 3) &&
                  (kkrt_psi_options.stash_size == 0),
              "now only support cuckoo HashNum = 3 , stash size = 0");
  ...
  PPU_ENFORCE(base_options.blocks.size() == 512,
              "now only support BaseSendOptions block size 512");
  ...
  for (size_t batch_idx = 0; batch_idx < ot_num_batch; ++batch_idx) {
    ...
    std::string send_str = receiver.ShiftCorrection(num_this_batch);
    // use throttle_control_sender
    throttle_control_sender.SendAsync(
        {send_str.c_str(), static_cast<int64_t>(send_str.size())}, tag);
  }
  throttle_control_sender.WaitRecvThread();
  ...
  while (true) {
    ...
    size_t curr_step_encode_num =
        curr_step_item_num * kkrt_psi_options.cuckoo_hash_num;
    PPU_ENFORCE_EQ(batch.flatten_bytes.size(),
