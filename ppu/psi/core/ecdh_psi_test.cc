...
TEST(EcdhPsiTestFailed, TargetRankMismatched) {
  for (std::pair<size_t, size_t> ranks : std::vector<std::pair<size_t, size_t>>{
           {0, 1}, {0, link::kAllRank}, {1, link::kAllRank}}) {
    auto ctxs = link::test::SetupWorld(2);
    auto proc = [&](std::shared_ptr<link::Context> ctx,
    ...
    ASSERT_THROW(fa.get(), EnforceNotMet);
    ASSERT_THROW(fb.get(), EnforceNotMet);
  }
}
...
TEST(EcdhPsiTestFailed, CurveTypeMismatched) {
  ...
  auto proc = [&](std::shared_ptr<link::Context> ctx,
    ...
    return RunEcdhPsi(ctx, items, link::kAllRank, type);
  };
...
TEST_P(EcdhPsiTest, Works) {
  ...
  auto ctxs = link::test::SetupWorld(2);
  auto proc =
      [&](std::shared_ptr<link::Context> ctx,
          const std::vector<std::string>& items) -> std::vector<std::string> {
    return RunEcdhPsi(ctx, items, params.target_rank);
  };
  ...
  if (params.target_rank == link::kAllRank || params.target_rank == 0) {
    EXPECT_EQ(results_a, intersection);
  } else {
    EXPECT_TRUE(results_a.empty());
  }
  if (params.target_rank == link::kAllRank || params.target_rank == 1) {
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        TestParams{{"a", "b"}, {"b", "c"}, link::kAllRank},  //
        TestParams{{"a", "b"}, {"b", "c"}, 0},               //
        TestParams{{"a", "b"}, {"b", "c"}, 1},               //
        //
        TestParams{{"a", "b"}, {"c", "d"}, link::kAllRank},  //
        TestParams{{"a", "b"}, {"c", "d"}, 0},               //
        TestParams{{"a", "b"}, {"c", "d"}, 1},               //
        //
        TestParams{{}, {"a"}, link::kAllRank},  //
        TestParams{{}, {"a"}, 0},               //
        TestParams{{}, {"a"}, 1},               //
        //
        TestParams{{"a"}, {}, link::kAllRank},  //
