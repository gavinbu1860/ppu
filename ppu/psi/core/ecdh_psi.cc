...
#include "ppu/psi/core/serializable.pb.h"
...
std::string CreateFlattenEccBuffer(const std::vector<std::string>& items,
  ...
  std::string ret;
  ret.reserve(items.size() * kEccKeySize);
  size_t size = std::min<size_t>(chosen_size, kEccKeySize);
  for (const auto& item : items) {
    PPU_ENFORCE(item.size() == kEccKeySize);
...
std::vector<std::string> CreateItemsFromFlattenEccBuffer(std::string_view buf) {
  PPU_ENFORCE(buf.size() % kEccKeySize == 0);
  size_t num_item = buf.size() / kEccKeySize;
  std::vector<std::string> ret;
  ret.reserve(num_item);
  for (size_t i = 0; i < num_item; i++) {
    ret.emplace_back(buf.data() + i * kEccKeySize, kEccKeySize);
  }
  return ret;
}
...
struct EcdhBatch {
  ...
  static EcdhBatch Deserialize(const Buffer& buf) {
    EcdhBatchProto proto;
...
struct RunContext {
  ...
  std::shared_ptr<link::Context> link0;
  std::shared_ptr<link::Context> link1;
  ...
  bool CanTouchResults() const {
    return this->options.target_rank == link::kAllRank ||
  ...
  std::vector<std::string> HashInputs(
    ...
    parallel_for(0, items.size(), 1, [&](int64_t begin, int64_t end) {
...
void RunMaskSelf(std::shared_ptr<RunContext> run_ctx) {
  ...
  while (true) {
    ...
    if (run_ctx->CanTouchResults()) {
      ...
      PPU_ENFORCE(run_ctx->window_cv.wait_until(
                      lock,
...
void RunMaskPeer(const std::shared_ptr<RunContext>& run_ctx,
  ...
  while (true) {
    const auto tag = fmt::format("ECDHPSI:X^A:{}", batch_count);
    EcdhBatch masked_batch =
        EcdhBatch::Deserialize(link0->Recv(link0->NextRank(), tag));
    // Fetch y^b.
    PPU_ENFORCE(masked_batch.flatten_bytes.size() % kHashSize == 0);
    size_t num_items = masked_batch.flatten_bytes.size() / kHashSize;

    // Compute (y^b)^a.
    EcdhBatch dual_masked_batch;
    ...
    if (num_items > 0) {
      ...
      for (size_t i = 0; i < num_items; ++i) {
        y_b.push_back(flatten_bytes.substr(i * kHashSize, kHashSize));
      }
...
void RunRecvPeer(const std::shared_ptr<RunContext>& run_ctx,
                 size_t dual_mask_size = kFinalCompareBytes) {
  PPU_ENFORCE(run_ctx->CanTouchResults());

  size_t batch_count = 0;
  while (true) {
    // Receive x^a^b.
    const auto tag = fmt::format("ECDHPSI:X^A^B:{}", batch_count);
    EcdhBatch batch = EcdhBatch::Deserialize(
        run_ctx->link1->Recv(run_ctx->link1->NextRank(), tag));

    const bool is_last_batch = batch.is_last_batch;
    PPU_ENFORCE(batch.flatten_bytes.size() % dual_mask_size == 0);
...
void RunEcdhPsi(const PsiOptions& options) {
  PPU_ENFORCE(options.link_ctx->WorldSize() == 2);
  PPU_ENFORCE(options.target_rank == link::kAllRank ||
  ...
  Buffer my_config_buf(my_config.c_str(), my_config.size());
  const auto* const tag = "ECDHPSI:SANITY";
  options.link_ctx->Send(options.link_ctx->NextRank(), my_config_buf, tag);
  auto peer_config = options.link_ctx->Recv(options.link_ctx->NextRank(), tag);
  PPU_ENFORCE(my_config_buf == peer_config, "Config mismatch, mine={}, peer={}",
              my_config, peer_config);
...
void RunMaskPeer(std::shared_ptr<RunContext> run_ctx, size_t recv_rank,
  ...
  while (true) {
    const auto tag = fmt::format("ECDHPSI:X^A:{}", batch_count);
    EcdhBatch masked_batch =
        EcdhBatch::Deserialize(link0->Recv(recv_rank, tag));
    // Fetch y^b.
    PPU_ENFORCE(masked_batch.flatten_bytes.size() % kHashSize == 0);
    size_t num_items = masked_batch.flatten_bytes.size() / kHashSize;
...
void RunRecvLinkRank(std::shared_ptr<RunContext> run_ctx, size_t recv_rank,
  ...
  while (true) {
    ...
    const auto tag = fmt::format("ECDHPSI:X^A^B:{}", batch_count);
    EcdhBatch batch =
        EcdhBatch::Deserialize(run_ctx->link1->Recv(recv_rank, tag));
...
std::shared_ptr<RunContext> CreateRunContext(
    const PsiOptions& options, const std::shared_ptr<link::Context>& link0,
    const std::shared_ptr<link::Context>& link1) {
  auto run_ctx = std::make_shared<RunContext>();
  run_ctx->options = options;
  run_ctx->link0 = link0;
  run_ctx->link1 = link1;
...
std::vector<std::string> HashInputs(const std::vector<std::string>& items) {
  ...
  parallel_for(0, items.size(), 1, [&](int64_t begin, int64_t end) {
    for (int64_t idx = begin; idx < end; ++idx) {
      std::vector<uint8_t> hash = crypto::Sha256(items[idx]);
