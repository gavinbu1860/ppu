...
CuckooIndex::CuckooIndex(const Options& options) : options_(options) {
  ...
  PPU_ENFORCE((options_.num_hash - 1) * HashRoom::kBlockSize +
                  sizeof(uint64_t) <=
              sizeof(HashType));
}
...
void CuckooIndex::Insert(absl::Span<const HashType> codes) {
  ...
  PPU_ENFORCE(hashes_.size() <= options_.num_input);
...
void CuckooIndex::PutToStash(uint64_t input_idx) {
  ...
  PPU_THROW("Cannot find empty bin in stash for input_idx={}", input_idx);
}
...
void CuckooIndex::SanityCheck() const {
  ...
  for (const auto& bin : bins_) {
    if (!bin.IsEmpty()) {
      PPU_ENFORCE(set.insert(bin.InputIdx()).second, "Input={} already exists.",
                  bin.InputIdx());
    }
  }
  ...
  PPU_ENFORCE(set.size() == options_.num_input);
  // Every input must exists.
  size_t idx = 0;
  for (uint64_t i : set) {
    PPU_ENFORCE(idx++ == i, "Cannot find input={}", i);
  }
}
...
CuckooIndex::Options CuckooIndex::SelectParams(uint64_t n, uint64_t stash_size,
  ...
  PPU_THROW("not support for stash_size={} and hash_num={}", stash_size,
            hash_num);
}
