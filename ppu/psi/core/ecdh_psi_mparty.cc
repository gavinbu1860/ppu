...
#include "ppu/psi/core/ecdh_psi_mparty.h"
...
EcdhPsiMParty::EcdhPsiMParty(
  ...
  PPU_ENFORCE(RAND_bytes(&private_key_[0], kKeySize) == 1,
...
void EcdhPsiMParty::RunMaskRecvAndForward(
    const std::shared_ptr<link::Context>& link, size_t recv_rank,
...
void EcdhPsiMParty::RunMaskSelfAndSend(
    const std::shared_ptr<link::Context>& link, size_t send_rank) {
  PPU_ENFORCE(link->Rank() != send_rank, "check send_rank({}) not self({})",
              send_rank, link->Rank());
...
void EcdhPsiMParty::RunRecvAndStore(const std::shared_ptr<link::Context>& link,
                                    size_t recv_rank, size_t dual_mask_size) {
  PsiOptions tmp_options = options_;
