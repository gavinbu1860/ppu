...
class CsvHeaderAnalyzer {
 public:
  CsvHeaderAnalyzer(const std::string& path,
    ...
    PPU_ENFORCE(in.is_open(), "Cannot open {}", path);
    std::string line;
    PPU_ENFORCE(std::getline(in, line), "Cannot read header line in {}", path);
    PPU_ENFORCE(!CheckIfBOMExists(line),
    ...
    for (const auto& target : target_set) {
      PPU_ENFORCE(col_index_map.find(target) != col_index_map.end(),
  ...
  static std::set<std::string> CheckAndNormalizeTokens(
    ...
    for (std::string input : inputs) {
      ...
      PPU_ENFORCE(!input.empty(),
                  "Found empty feature name, input feature names='{}'",
                  fmt::join(inputs, ","));
      ret.insert(input);
    }
    PPU_ENFORCE(ret.size() == inputs.size(), "Repeated feature name in ='{}'",
                fmt::join(inputs, ","));
    return ret;
  }
