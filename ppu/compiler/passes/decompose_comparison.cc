...
struct CompareOpConverter : public OpRewritePattern<CompTy> {
  ...
  LogicalResult matchAndRewrite(CompTy op,
    ...
    auto shape = op.getType().template dyn_cast<RankedTensorType>().getShape();
    auto eq_op =
    ...
    auto const_op = builder.create<ConstOp>(
        ...
        DenseIntElementsAttr::get(
            RankedTensorType::get(shape, builder.getIntegerType(32)), 1));

    rewriter.replaceOpWithNewOp<SubOp>(op, op.getType(), const_op, eq_op);
