...
from ppu.binding.test_util import PropagatingThread
...
class UnitTests(unittest.TestCase):
    def run_psi(self, fn):
        ...
        jobs = [
            PropagatingThread(target=wrap, args=(rank, ))
            for rank in range(wsize)
        ]
    ...
    def run_streaming_psi(self, fn, wsize, inputs, outputs, selected_fields):
        ...
        jobs = [
            multiprocessing.Process(target=wrap,
                                    args=(
                                        rank,
                                        selected_fields,
                                        inputs[rank],
    ...
    def prep_data(self):
        ...
        expected = [
            f'r{idx}' for idx in range(1000) if idx % 3 == 0 and idx % 7 == 0
        ]
    ...
    def test_ecdh_3pc(self):
        ...
        inputs = [
            "ppu/binding/data/alice.csv", "ppu/binding/data/bob.csv",
            "ppu/binding/data/carol.csv"
        ]
        outputs = [
            "./alice-ecdh3pc.csv", "./bob-ecdh3pc.csv", "./carol-ecdh3pc.csv"
        ]
        selected_fields = ["id", "idx"]

        def fn(lctx, selected_fields, input_path, output_path):
            report = libs.PsiReport()
            libs.ecdh_3pc_psi(lctx, selected_fields, input_path, output_path,
                              True, report)
    ...
    def test_kkrt_2pc(self):
        ...
        inputs = ["ppu/binding/data/alice.csv", "ppu/binding/data/bob.csv"]
        outputs = ["./alice-kkrt.csv", "./bob-kkrt.csv"]
        selected_fields = ["id", "idx"]

        def fn(lctx, selected_fields, input_path, output_path):
            report = libs.PsiReport()
            libs.kkrt_2pc_psi(lctx, selected_fields, input_path, output_path,
                              True, report)
            source_count = wc_count(input_path)
            output_count = wc_count(output_path)
            id = lctx.id()
    ...
    def test_ecdh_2pc(self):
        ...
        def fn(lctx, selected_fields, input_path, output_path):
            report = libs.PsiReport()
            libs.ecdh_2pc_psi(lctx, selected_fields, input_path, output_path,
                              64, True, report)
            source_count = wc_count(input_path)
            output_count = wc_count(output_path)
            id = lctx.id()
