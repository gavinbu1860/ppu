    ...
    for (size_t idx = 0; idx < descs.size(); idx++) {
      ...
      if (rank == 0) {
        ...
        if (op == RecOp::ADD) {
          ...
          PPU_ENFORCE("not supported reconstruct op");
        }
      }
    }
  }
...
void checkDescs(absl::Span<const PrgArrayDesc> descs) {
  for (size_t idx = 1; idx < descs.size(); idx++) {
    PPU_ENFORCE(descs[0].field == descs[idx].field);
    PPU_ENFORCE(descs[0].numel == descs[idx].numel);
  }
}
...
void TrustedParty::setSeed(size_t rank, size_t world_size,
                           const PrgSeed& seed) {
  PPU_ENFORCE(rank < world_size, "rank={} should be smaller then world_size={}",
              rank, world_size);
  ...
  if (seeds_.size() == 0) {
    ...
    PPU_ENFORCE(world_size == seeds_.size(),
                "parties claim different world_size, prev={}, cur={}",
                seeds_.size(), world_size);

    PPU_ENFORCE(!seeds_[rank].has_value() || seeds_[rank].value() == seed);
...
std::vector<PrgSeed> TrustedParty::getSeeds() const {
  ...
  for (size_t rank = 0; rank < seeds_.size(); rank++) {
    PPU_ENFORCE(seeds_[rank].has_value(), "seed for rank={} not set", rank);
...
ArrayRef TrustedParty::adjustDot(absl::Span<const PrgArrayDesc> descs, size_t M,
                                 size_t N, size_t K) {
  PPU_ENFORCE_EQ(descs.size(), 3u);
  PPU_ENFORCE(descs[0].numel == M * K);
  PPU_ENFORCE(descs[1].numel == K * N);
  PPU_ENFORCE(descs[2].numel == M * N);
...
ArrayRef TrustedParty::adjustRandBit(const PrgArrayDesc& desc) {
  ...
  PPU_ENFORCE(r0.size() == 1 && rs.size() == 1);
