...
class BShrTy : public TypeImpl<BShrTy, RingTy, Secret, BShare> {
  ...
  // Theoretically, `B share` works in F2 (mod 2) ring, which use a single bit
  // to represent a boolean share.
  //
  // But just like plaintext data (which process multi-bits together in byte),
  // PPU also encode multi-bits (F2) together. This member represent the number
  // of valid bits for a multi-bits storage.
  static constexpr size_t kDefaultNumBits = std::numeric_limits<size_t>::max();
  // the number=nbit_ of LSB are valid.
  size_t nbits_ = kDefaultNumBits;
