...
// for example:
//   auto npc = 2;
//   auto fn = [](const std::shared_ptr<link::Context>& lctx) {
//     balabala...
//   }
//   simulate(npc, fn);
//
template <typename Fn, typename... Args,
          typename R = std::invoke_result_t<
              Fn, const std::shared_ptr<link::Context>&, Args...>,
          std::enable_if_t<!std::is_same_v<R, void>, int> = 0>
std::vector<R> simulate(size_t npc, Fn&& fn, Args&&... args) {
  auto lctxs = link::test::SetupWorld(fmt::format("sim.{}", npc), npc);
