...
// this file defines the basic types use in ppu vm.
...
#define __CASE_PT_TYPE(PT_TYPE, NAME, ...)                       \
  case (PT_TYPE): {                                              \
    ...
    [[maybe_unused]] constexpr ppu::PtType _kPtType = PT_TYPE;   \
...
#define DISPATCH_ALL_PT_TYPES(PT_TYPE, NAME, ...)                       \
  [&] {                                                                 \
    switch (PT_TYPE) {                                                  \
      __CASE_PT_TYPE(ppu::PT_I8, NAME, __VA_ARGS__)                     \
      __CASE_PT_TYPE(ppu::PT_U8, NAME, __VA_ARGS__)                     \
      __CASE_PT_TYPE(ppu::PT_I16, NAME, __VA_ARGS__)                    \
      __CASE_PT_TYPE(ppu::PT_U16, NAME, __VA_ARGS__)                    \
      __CASE_PT_TYPE(ppu::PT_I32, NAME, __VA_ARGS__)                    \
      __CASE_PT_TYPE(ppu::PT_U32, NAME, __VA_ARGS__)                    \
      __CASE_PT_TYPE(ppu::PT_I64, NAME, __VA_ARGS__)                    \
      __CASE_PT_TYPE(ppu::PT_U64, NAME, __VA_ARGS__)                    \
      __CASE_PT_TYPE(ppu::PT_F32, NAME, __VA_ARGS__)                    \
      __CASE_PT_TYPE(ppu::PT_F64, NAME, __VA_ARGS__)                    \
      __CASE_PT_TYPE(ppu::PT_I128, NAME, __VA_ARGS__)                   \
      __CASE_PT_TYPE(ppu::PT_U128, NAME, __VA_ARGS__)                   \
      default:                                                          \
        PPU_THROW("{} not implemented for pt_type={}", #NAME, PT_TYPE); \
    }                                                                   \
  }()
...
#define __CASE_FIELD(FIELD, NAME, ...)                                \
  case (FIELD): {                                                     \
    ...
    [[maybe_unused]] constexpr ppu::FieldType _kField = FIELD;        \
    [[maybe_unused]] constexpr std::string_view _kName = NAME;        \
...
#define DISPATCH_ALL_FIELDS(FIELD, NAME, ...)                       \
  [&] {                                                             \
    switch (FIELD) {                                                \
      __CASE_FIELD(ppu::FieldType::FM32, NAME, __VA_ARGS__)         \
      __CASE_FIELD(ppu::FieldType::FM64, NAME, __VA_ARGS__)         \
      __CASE_FIELD(ppu::FieldType::FM128, NAME, __VA_ARGS__)        \
      default:                                                      \
        PPU_THROW("{} not implemented for field={}", #NAME, FIELD); \
    }                                                               \
  }()
