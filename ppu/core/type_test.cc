...
TEST(TypeTest, PublicTy) {
  {
    Type ty = makeType<Ring2kPublTy>(FM32);
    EXPECT_EQ(ty.size(), 4);
    ...
    EXPECT_EQ(ty.toString(), "Ring2kPubl<FM32>");

    EXPECT_EQ(Type::fromString(ty.toString()), ty);
  }

  {
    Type ty = makeType<Ring2kPublTy>(FM128);
    ...
    EXPECT_EQ(ty.toString(), "Ring2kPubl<FM128>");
