...
struct SimdTrait<
  ...
  static OutputIt unpack(const C& v, OutputIt result, const PackInfo& pi) {
    ...
    PPU_ENFORCE(v.size() == total_num, "split number mismatch {} != {}",
                v.size(), total_num);
...
OutputIt vectorize(InputIt a_first, InputIt a_last, InputIt b_first,
  ...
  PPU_ENFORCE(a_pi == b_pi, "join info mis-match, a_pi={}, b_pi={}",
              a_pi.size(), b_pi.at(2));
...
template <typename T>
using ConcatFn = std::function<T(absl::Span<const T> vs)>;

template <typename T>
using SplitFn = std::function<std::vector<T>(const T&, size_t)>;

template <typename T>
using BinaryFn = std::function<T(const T&, const T&)>;

template <typename T>
using VectorizeFn = std::function<std::vector<T>(absl::Span<const T> lhs,
                                                 absl::Span<const T> rhs)>;

// Given T as a splittable/concatenable type, this function operates on a series
// of objects with only one binary op instance.
template <typename T>
std::vector<T> Vectorize(absl::Span<const T> lhs, absl::Span<const T> rhs,
                         const ConcatFn<T>& concat, const SplitFn<T>& split,
                         const BinaryFn<T>& binary_op) {
  PPU_ENFORCE(lhs.size() == rhs.size(),
              "vectorize op length mismatch, lhs={}, rhs={}", lhs.size(),
              rhs.size());

  const size_t len = lhs.size();
  return split(binary_op(concat(lhs), concat(rhs)), len);
}
...
// use n-1 operations.
//
// This is especially useful for latency intensive operations.
template <typename T>
T VectorizedReduce(absl::Span<const T> vs, const VectorizeFn<T>& vec_op) {
  PPU_ENFORCE(vs.size() > 0, "tree reduce size={} should >0", vs.size());

  std::vector<T> cur(vs.begin(), vs.end());
  while (cur.size() > 1) {
    size_t len = cur.size() / 2;

    absl::Span<T const> cur_span = cur;
    std::vector<T> next =
        vec_op(cur_span.subspan(0, len), cur_span.subspan(len, len));
