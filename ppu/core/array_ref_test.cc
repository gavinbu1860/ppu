...
TEST(ArrayRefTest, Simple) {
  ...
  auto mem = makeBuffer(kBufSize);

  ArrayRef a(mem, I32, 100 / 4, 4, 0);

  EXPECT_EQ(a.numel(), kBufSize / I32.size());
  EXPECT_EQ(a.stride(), I32.size());
  EXPECT_EQ(a.elsize(), I32.size());
  EXPECT_EQ(a.offset(), 0);

  ArrayRef b(mem, I32, 10, 8, 20);
  EXPECT_EQ(b.numel(), 10);
  EXPECT_EQ(b.stride(), 8);
...
TEST(ArrayRefTest, Strides) {
  ...
  ArrayRef a(makeBuffer(6 * sizeof(int32_t)), makePtType(PT_I32), 3, 2, 0);
...
TEST(ArrayRefTest, NdStrides) {
  ...
  NdArrayRef a(makeBuffer(36 * sizeof(int32_t)), makePtType(PT_I32), {3, 3},
               {2 * 6, 2}, 0);
