...
.. warning::
   This is a design draft for using PPU from secretflow, not an accurate document for PPU itself. It could be treated as a reference to integrate PPU from other system.
...
.. code-block:: yaml
    ...
    devices: # list of devices
      - {device-name}    # the ith device name
        type: {dev-type} # the type of the device, one of {PPU, HEU, CPU, TEE}
        nodes: {composition-nodes}  # this is device type dependent, PPU=list of node, CPU=single node, HEU=node pair.
...
.. code-block:: yaml
    ...
    devices:
      - PPU0: # the first one is PPU, which use three nodes
        type: PPU
        nodes: [ "N1", "N2", "N3" ] 
      - P1:
        type: CPU
...
Let's ignore the PPU device for a moment, the CPU and HEU looks like this:
...
- First, transfer data from `P1` to `HEU`, in this case, from device concept level, `P1` **strong trust** on `HEU`, so it can send cleartext directly to `HEU`. In implementation, `P1` colocated with `HEU`, so it make sense for a cleartext transfer.
